...
class RandomCrop_getparam(Transform):
    ...
    def __call__(self, image: torch.Tensor) : # return cropped image and a coord where the crop starts in original image
        ...
        image_1 = torchvision.transforms.functional.crop(image[0], i, j, h, w)
        image_2 = torchvision.transforms.functional.crop(image[1], i, j, h, w)

        return torch.stack([image_1, image_2], dim=0), (i,j)


        # print(len(crop_images), crop_images[0].shape)  
        # 10 torch.Size([2, 3, 164, 164])

        
                  
        # print(image_resized.shape, image.shape)
        # torch.Size([3, 164, 164]) torch.Size([3, 370, 1226])

        # print(min(image_resized.reshape(-1)), max(image_resized.reshape(-1)))
        # print(min(image.reshape(-1)), max(image.reshape(-1)))
        # tensor(0, dtype=torch.uint8) tensor(255, dtype=torch.uint8)
        # tensor(0, dtype=torch.uint8) tensor(255, dtype=torch.uint8)

        # return crop_images, image, img_sh, self.cropsize 
