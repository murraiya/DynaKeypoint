...
def load_images(*paths, as_gray=True):
    ...
    ori_images = np.stack([io.imread(path, as_gray=False) for path in paths])

    images = torch.tensor(images, device=DEVICE, dtype=torch.float32)
    ori_images = torch.tensor(ori_images, device=DEVICE, dtype=torch.float32)
    # if not as_gray:
    ori_images = ori_images.permute(0, 3, 1, 2)
        # images = images / 255.0
    # else:
    images = images.unsqueeze(1)  # add channel dimension
    return images, ori_images
