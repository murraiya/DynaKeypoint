...
class kittiMOTdataset(data.Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        img1_gray = io.imread(sample["img_path"][0][0], as_gray=True)         
        img2_gray = io.imread(sample["img_path"][0][1], as_gray=True)  
        ...
        # return np.stack([img1,img2], axis=0), sample["rel_pose"], sample["intrinsics"], depth1, depth2
        return np.stack([img1,img2], axis=0), np.expand_dims(np.stack([img1_gray,img2_gray], axis=0), axis=3), sample["rel_pose"][0], sample["intrinsics"]
